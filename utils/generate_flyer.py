...
def getTemplate_mm_A4LANDSCAPEINSIDE():
    #TEMPLATE
...
def getData():
    #DATA
    dados = [
            {'id': 1, 'janela_id': 'janela_1', 'titulo': 'Conjunto de Blazer e Calça', 'subtitulo': 'Oferta impertível da marca X&X', 'descricao': 'Conjunto esporte para ocasiões especiais com corte moderno para uso casual chic', 'valor': 'De R$1800,00 por R$1290,00', 'nome_loja': 'SXER', 'endereco': 'Al Macucos 135 Sao Paulo SP 456 Bairro dos Pratos', 'imagem_path': 'assets/imagens/img2.png', 'logo_path': 'assets/logos/logo03.jpg'},
            {'id': 2, 'janela_id': 'janela_2', 'titulo': 'Terno Esportivo Azul', 'subtitulo': 'Grande Promoção 30% de desconto', 'descricao': 'Terno completo com colete corte inglês em gabardine', 'valor': 'De R$3300,00 por R$2199,00', 'nome_loja': 'Dr. Terno', 'endereco': 'Al Gretyuter 3455 Sao Paulo SP ', 'imagem_path': 'assets/imagens/img3.png', 'logo_path': 'assets/logos/logo04.jpg'},
            {'id': 3, 'janela_id': 'janela_3', 'titulo': 'Carteira Pelica', 'subtitulo': 'Oferta especial desconto de 50%', 'descricao': 'Carteira de festa em pelica rosa para seu sucesso criata pelo estilista JJFred. Diponível nas cores rosa, edverde , azul e creme', 'valor': 'De R$2300,00 por R$1199,00', 'nome_loja': 'Sereníssima', 'endereco': 'Al Macucos 135 Sao Paulo SP CEP 04654-000', 'imagem_path': 'assets/imagens/img4.png', 'logo_path': 'assets/logos/logo05.jpg'}, 
            {'id': 4, 'janela_id': 'janela_4', 'titulo': 'Vestido em Seda Pura', 'subtitulo': 'Oportunidade', 'descricao': 'Vestido vinho de festa, em seda pura importada de finíssima qualidade', 'valor': 'De R$989,00 por R$788,00', 'nome_loja': 'Veste Sinhá', 'endereco': 'Al ghyu 334 Sao Paulo SP ', 'imagem_path': 'assets/imagens/img1.png', 'logo_path': 'assets/logos/logo1.png'},
...
def draw_window(windowData ):
    ...
    def draw_windowBackground(w,h,radius,outline,width,bg_pattern):
        back=create_background(bg_pattern,w,h)
        background_RGBA = back.convert("RGBA")

        # Create a transparent overlay to draw the rounded rectangle
        overlay = Image.new("RGBA", background_RGBA.size, (255, 255, 255, 0))
        draw = ImageDraw.Draw(overlay)
        draw.rounded_rectangle((0,0,w,h), radius=radius, outline=outline, width=width, fill=(255, 255, 255, 128))

        # Composite the overlay onto the background
        window_background = Image.alpha_composite(background_RGBA, overlay)
        return window_background

    w, h = windowData['w'], windowData['h']
    window_pattern_path=windowData['window_pattern_path']
    radius = 20
    window_background=draw_windowBackground(w,h,radius,"black",2,window_pattern_path)
    ...
    logo_h = int(h * 0.24)
    logo_w = logo_h
    img = Image.open(windowData["imagem_path"]).resize((img_w, img_h))
    logo = Image.open(windowData["logo_path"]).resize((logo_w, logo_h))
    window_background.paste(img,(10,10))
    window_background.paste(logo,(10,20+img_h,))

    # Text section
    font_regular = ImageFont.truetype("arialbd.ttf", 36)
    font_bold = ImageFont.truetype("arialbd.ttf", 48)
    draw = ImageDraw.Draw(window_background)

    # titulo
    ty=10
    tx=img_w+20
    max_font=font_bold
    text=windowData["titulo"]
...
# === MAIN GENERATOR ===
def generate_flyer(dados, template_px, pattern_path, window_pattern_path,PAGE_WIDTH,PAGE_HEIGHT):
    ...
    for item in dados:
        ...
        if janela_id not in template_px:
            continue
        template=template_px[janela_id]
        windowData= {**item,**template,'window_pattern_path':window_pattern_path}
        x, y=windowData['x'], windowData['y']
